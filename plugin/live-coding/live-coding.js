...
var LiveCoding = (function () {
    ...
    function update(codeElement) {
        ...
        if (isCSS) {
            ...
            // replace content
            switch (demoElement.nodeName) {
                default:
                    demoElement.innerHTML = codeElement.textContent;
                    break;

                case 'IFRAME':
                    demoElement.src = `data:text/html,<html><style>html, body { margin: 0; padding: 0; }
img { max-width: 100%; height: auto; vertical-align: bottom }</style><body>${codeElement
                        .textContent
                        .replace(/images/g, `${window.location.protocol}${window.location.host}${window.location.pathname}/images`)
                        .replace(/"/g, "'")
                        .replace(/</g, '%3C')
                        .replace(/>/g, '%3E')
                        .replace(/#/g, '%23')}</body></html>`;
                    break;
            }
        }
